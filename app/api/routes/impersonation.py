...
async def impersonate_policyholder(
    ...
    """
    Generate a policyholder token for the main app. Allows the superadmin
    to log into the client frontend as a policyholder.
    """
    # Verify tenant
    t_result = await db.execute(select(Tenant).where(Tenant.id == tenant_id))
    tenant = t_result.scalar_one_or_none()
    ...
    ph_result = await db.execute(
        select(Policyholder).where(
            ...
            Policyholder.policy_number == body.policy_number.strip(),
    ...
    if not policyholder:
        raise HTTPException(
            status_code=404,
            detail=f"No active policyholder with policy '{body.policy_number}' in this tenant",
        )

    # Create a token matching the main app's policyholder token format
    payload = {
        "sub": policyholder.policy_number,
        "tenant_id": str(tenant_id),
        "role": "policyholder",
    ...
    display_name = policyholder.last_name or policyholder.company_name or policyholder.policy_number

    await _log_action(db, admin, "impersonation.policyholder", "policyholder", str(policyholder.id), {
        "tenant_id": tenant_id,
        "tenant_name": tenant.name,
    ...
    return ImpersonationToken(
        ...
        tenant_id=str(tenant_id),
        tenant_name=tenant.name,
        user_identifier=policyholder.policy_number,
        role="policyholder",
        expires_in_hours=IMPERSONATION_HOURS,
        notice=f"Impersonation token for policyholder {display_name} ({policyholder.policy_number}). "
               f"Valid for {IMPERSONATION_HOURS} hours. All actions are logged.",
    )
