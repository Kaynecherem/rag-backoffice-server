...
from app.models.database import (
    Tenant, StaffUser, Policyholder, Document, QueryLog, AuditLog,
)
from app.models.schemas import (
    PlanConfig, TenantPlanAssign, TenantUsage, PlatformUsageSummary,
)
...
# ── Plan definitions (stored in code for now, DB-backed later) ────────

PLANS = {
    "trial": PlanConfig(
        name="Trial", query_limit_monthly=100, document_limit=20,
        staff_limit=2, policyholder_limit=50, features=["widget"],
    ),
    "starter": PlanConfig(
        name="Starter", query_limit_monthly=1000, document_limit=100,
        staff_limit=5, policyholder_limit=500, features=["widget", "batch_upload"],
    ),
    "professional": PlanConfig(
        name="Professional", query_limit_monthly=10000, document_limit=500,
        staff_limit=20, policyholder_limit=5000, features=["widget", "batch_upload", "api_access"],
    ),
...
async def _get_tenant_usage(db: AsyncSession, tenant, period: str) -> TenantUsage:
    ...
    return TenantUsage(
        ...
        documents_count=doc_count, documents_limit=plan.document_limit,
...
async def assign_plan(
    ...
    old_plan = getattr(tenant, "plan", "trial") or "trial"
    tenant.plan = body.plan

    await _log_action(db, admin, "billing.plan_change", "tenant", tenant_id, {
        "from": old_plan, "to": body.plan,
    }, request)
    await db.commit()
...
async def platform_usage_summary(
    ...
    """Platform-wide usage summary for the current month."""
    now = datetime.utcnow()
    period = now.strftime("%Y-%m")
    start = datetime(now.year, now.month, 1)

    total_queries = (await db.execute(
        select(func.count(QueryLog.id)).where(QueryLog.queried_at >= start)
    )).scalar() or 0
    ...
    at_risk = []
    for t in tenants:
        usage = await _get_tenant_usage(db, t, period)
        usage_by_tenant.append({
            "tenant_id": usage.tenant_id, "tenant_name": usage.tenant_name,
            "plan": usage.plan, "queries_used": usage.queries_used,
            "queries_limit": usage.queries_limit, "usage_pct": usage.usage_pct,
        })
        if usage.at_risk:
            at_risk.append({"tenant_id": usage.tenant_id, "tenant_name": usage.tenant_name, "usage_pct": usage.usage_pct})

    return PlatformUsageSummary(
        total_queries_this_month=total_queries, total_documents=total_docs,
        tenants_at_risk=at_risk, usage_by_tenant=usage_by_tenant,
    )
